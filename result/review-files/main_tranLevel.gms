...
$GDXIN ./data/intMatrix5.gdx
$LOAD B
$GDXIN
B(k,j) = round(B(k,j), 6);

$GDXIN ./data/CharacFactors3.gdx
$LOAD C
$GDXIN
C(l,k) = round(C(l,k), 2);
...
$if not set s140labs $set s140labs 1000
$if not set s142pyldpe $set s142pyldpe 1000
$if not set s143pyhdpe $set s143pyhdpe 1000
$if not set s144pypp $set s144pypp 1000
$if not set s147claypla $set s147claypla 1000
$if not set s149acidpla $set s149acidpla 1000
$if not set s150alcpla $set s150alcpla 1000
$if not set s151wind $set s151wind 0
$if not set s152solar $set s152solar 0
$if not set s153bioet $set s153bioet 1000
...
variable tlnum;
equation eqtlnum;
eqtlnum.. tlnum=e=costIn+costRecycled+costCl+costLu+costPy+cost_inn+cost_etFromDAC;

variable gwp;
gwp.lo=-1000;
equation eq_gwp;
*eq_gwp.. gwp*1.56*(cost)=e=(mp_indicators('MPI4')-0.15*s('P116'))*96;
eq_gwp.. gwp*1.56*(tlnum+(productionCostResin))=e=(mp_indicators('MPI4')-0.15*s('P116'))*15;
...
execute 'python ./src/hotspotFinder.py scalingVector.csv'
