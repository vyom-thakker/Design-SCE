...
parameter s_initial(j),techMat_initial(i,j);
...
$GDXIN techMat_initial.gdx
$LOAD techMat_initial
$GDXIN
...
techMat.l(i,j)$(HCon(i,j)+SWast(i,j))=techMat_initial(i,j);
...
eqs6.. techMat('E128','P92')*sum(j$bagAmnts(j), s(j))=e=s('P132')*(1-techMat('E128','P92'));
...
DoC_obj.. DoC*productionCostResin =e=costIn +costRecycled+costBenifitCompost+costCl+costLu+costPy;
...
cd.fx('Paper','Households')=s.l('P134')*(-1)*(techMat.l('E125','P134')+techMat.l('E126','P134'));
...
cd.fx('Segregation','Clinker')=s.l('P92')*techMat.l('E97','P92');
...
s_initial(j)=s.l(j);
techMat_initial(i,j)=techMat.l(i,j);
execute_unload  's_initial.gdx', s_initial;
execute_unload 'techMat_initial.gdx',techMat_initial;
