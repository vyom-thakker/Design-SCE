...
*s.fx('P85')=0;
*s.fx('P86')=0;
set restrictEpsilons(i,j) /#HCon,#SWast/;
epsilons1.fx(i,j)$restrictEpsilons(i,j)=0;
epsilons2.fx(i,j)$restrictEpsilons(i,j)=0;

equation techmat_define(i,j);
techmat_define(i,j)$(ij(i,j)-HCon(i,j)-SWast(i,j)).. techMat(i,j)=e=A(i,j)*(1+%delta%*epsilons1(i,j)-%delta%*epsilons2(i,j));
...
*sum(j$ifill_indices(j),s(j)*907.18*(1-(((techMat('E107',j))*elecCost/360)/offsetCost(j))))+sum(j$ifill_indices_new(j),s(j)*907.18*(1-(((techMat('E107',j))*elecCost/360)/offsetCost_new(j))));
...
loss_b.. lossBioFuel*normalizedCostInput=g=0;
lossBioFuel.lo=0;
