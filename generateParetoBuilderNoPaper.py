...
df=pd.read_csv("pareto"+sys.argv[2]+".txt", sep=',',header =None,names=['Cost','DoC','MassConsumed','HDPE','LDPE','PP','PLA','PHA','Reprocess','Pyrolysis','Landfill','Incineration','GWP'])
...
x1=np.linspace(min(df['GWP'])*1.005,max(df['GWP'])*0.995,int(sys.argv[3]));
...
for x in X[0]:
    for y in Y:
        ...
        commands.extend("gams RPE"+di+".gms --gwpC="+str(round(x,4))+" --costC="+str(round(y[0],4))+" --file=$file --fileS="+str(k)+"$file --fileS1="+str(k)+"g$file parmfile=moreOptions.txt \n");
        k=k+1;
