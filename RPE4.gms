...
*NoPaper
*Only Plastic
*Needs modification of hotspot.py file for hotspot detection
sets
	i substances /E1*E123/
	j activities /P1*P130/
	k impacts /I1*I1545/
    l MPindicators /MPI1*MPI10/
	losses(i) /E97/
	intermediates(i) /E1*E77,E78*E82,E84*E89,E91*E96,E107,E113,E115,E117*E122/
	homes(j) homesubsets /P87/
	supplies(i) bagsperhome /E84*E88/
	sortedStuff(i) sortedbagsweights /E92*E96/
	unextrudedAmnts(j) bagspermake /P77*P81/
    bagAmnts(j) extruded plastic /P82*P86/
...
parameter 
	bagVolumes(i) volume /E84 13.75, E85 [29.3*%rL%], E86 [43.3*%rpp%], E87 [14*%rpla%], E88 14/
...
*sorting
equations eqs1,eqs2,eqs3,eqs4,eqs5,eqs6;
...
eqs6..907.18*s('P106')=e=5*s('P92')*(1-techMat('E97','P92'));
...
equation costInput;
set bagRatioProduction(i) /E92*E96/;
*parameter offsetCostInput(i) /E92 8440,E93 8440,E94 3740,E95 1940,E96 4521/;
parameter offsetCostInput(i) /E92 900,E93 990,E94 1080,E95 1240,E96 2500/;
...
*landfilling
set lfill_indices(j) /P98*P102/;
variable lossLandfill;
equations eql1,eql2,eql3,eql4,eql5,eql6,loss_l;
...
eql6.. s('P103')=e=5*sum(j$lfill_indices(j), s(j));
loss_l.. lossLandfill=e=907.18*sum(j$lfill_indices(j),s(j));

s.fx('P81')=0;
*incineration
set ifill_indices(j) /P108*P112/;
*parameter offsetCost(j) /P108 8440,P109 8440,P110 3740,P111 1940, P112 4521/;
parameter offsetCost(j) /P108 900,P109 990,P110 1080,P111 1240, P112 2500/;
set ifill_indices_new(j) /P118*P121/;
parameter offsetCost_new(j) /P118 900,P119 990,P120 1080,P121 1940/;
variable lossIncineration;
equations eqi1,eqi2,eqi3,eqi4,eqi5,eqi6,loss_i;
...
eqi6.. s('P114')=e=5*sum(j$ifill_indices(j), s(j));
...
loss_i.. lossIncineration=e=sum(j$ifill_indices(j),s(j)*907.18*(1-(((techMat('E107',j))*10/360)/offsetCost(j))))+sum(j$ifill_indices_new(j),s(j)*907.18*(1-(((techMat('E107',j))*10/360)/offsetCost_new(j))));

*biofuel
variable lossBioFuel;
equation loss_b;
set bio_new(j) /P122*P125/;
loss_b.. lossBioFuel*normalizedCostInput=e=(((-1*s('P115')/techMat('E91','P115')+sum(j$bio_new(j),s(j)))*normalizedCostInput)-(s('P116')*0.041));
lossBioFuel.lo=0;
...
cost_c.. costBenifitCompost=e=(s('P117')+s('P104')+s('P105'))*907.18*0.030183;
...
costRecycled1.. costRecycled*0.90718=e=A('E81','P96')*s('P96')*1.24+A('E80','P95')*s('P95')*1.08+A('E79','P93')*s('P93')*0.9+A('E78','P94')*s('P94')*0.99;

variable costIn,costPy,costCl,costLu;
equation costIn1,costPy1,costCl1,costLu1;

costIn1.. costIn=e=sum(j$ifill_indices(j),s(j)*techMat('E107',j)*10/360)+sum(j$ifill_indices_new(j),s(j)*techMat('E107',j)*10/360);
costPy1.. costPy=e=s('P116')*0.041;
costCl1.. costCl=e=s('P129')*34/1000;
*clinker 34$/ton, lumber=45$/1000 board feet i.e. 2.35 m3 and 320 kg/m3 density
costLu1.. costLu=e=s('P130')*0.036;
*pyrolysis 0.044 kg biofuel generated, 7.3 lb/gal density, 3$/gal
*degreeofcircularity
*DoC_obj.. DoC*sum(j$bagAmnts(j), s(j))=e=sum(j$bagAmnts(j), s(j))-(f('E97')+lossLandfill);
...
DoC_obj.. DoC*(productionCostResin+costRecycled) =e=costIn +costRecycled+costBenifitCompost+costPy+costCl+costLu;
...
If(docC<0, Solve ToyProblem Using NLP maximizing epsObj; 
zD = DoC.l;
DoC.lo=zD;
