...
set lossRow(i) /E97/;
parameter regenFactGuess(lossRow,j);
$offlisting
$include regenFactGuess.inc
$onlisting
set aggWasteLoss(j) /P87*P92,P98*P102,P108*P112,P115/;
positive variable regenFact(lossRow,j);
set lossMat /#j/;
regenFact.fx(lossRow,j)$(lossMat(j)-aggWasteLoss(j))=regenFactGuess(lossRow,j);
parameter absWaste(j) /P82 0.995,P83 0.995,P84 0.94, P85 0.4,P86 0.3,P132 0.15/;
parameter absLandfill(j) /P82 0.9,P83 0.9,P84 0.95, P85 0.2,P86 0.3,P132 0/;
...
C(l,'I1765')=0;
*No Co2 sequestered positive impact
...
equations
    ...
    absLoss  collected waste
    absLF
    absLoss1(aggWasteLoss) regenFactCollected;

positive variables aggWasteLossVal,aggLFval;
    ...
    absLoss.. aggWasteLossVal*sum(j$bagAmnts(j), s(j)) =e= sum(j$bagAmnts(j), s(j)*absWaste(j));
    absLoss1(aggWasteLoss(j)).. regenFact('E97',j) =e= aggWasteLossVal;
    absLF.. aggLFval*sum(j$bagAmnts(j), s(j)) =e= sum(j$bagAmnts(j), s(j)*absLandfill(j));
...
cd.fx('Segregation','Incineration')=sum(j$ifill_indices_new(j), s.l(j)*(907.18-techMat.l('E97',j)));
